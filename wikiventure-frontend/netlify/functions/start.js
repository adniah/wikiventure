...
exports.handler = async (event) => {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return { statusCode: 500, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'MISSING_API_KEY' }) };
  }
...
NOTE: When the story reaches a natural conclusion, you can end it by including "ENDING: true" and "ACHIEVEMENT: [creative achievement name]" instead of providing choices.`;
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'Failed to generate story', status: error.status, message: error.message || String(error) })
    };
  }
};
